...
class TsForecastingMultiStepTFEnv(tf_environment.TFEnvironment):
    def __init__(self, ts_data, rl_algorithm, data_summary, initial_state_val=0.0, pred_horizon=6, window_size=6,
        ...
        self._initial_state_val = initial_state_val
    ...
    def get_environment_with_state_size(self, state_size):
        # return new environment with only the state size changed
        return TsForecastingMultiStepTFEnv(
            self.ts_data,
            self._rl_algorithm,
            self._data_summary,
            self._initial_state_val,
            self._pred_horizon,
            self.window_size,
            self.max_window_count,
            self.min_attribute_val,
            self.max_attribute_val,
            self._batch_size,
            self.use_rnn_state,
            state_size,
            self.use_pred_diff,
