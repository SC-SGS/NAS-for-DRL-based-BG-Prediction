            ...
            if num_gpus > 0:
                for gpu_id in range(num_gpus):
                    ...
                    eval_results = evaluation.compute_metrics_multi_step(
                        ...
                        "last", log_dir, metrics=["rmse"], prefix="test", save_file=False, analyze_hw_performance=True
                    )
                    ...
                    logging.info(
                        "Time consumption per step (inference): {}".format(eval_results[-2] / eval_results[-1]))
            else:
                ...
                logging.info("Time consumption per step (inference): {}".format(eval_results[-2] / eval_results[-1]))
        else:
            import pylikwid
            cpus = []
