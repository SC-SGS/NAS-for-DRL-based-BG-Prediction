...
from utils import clarke_error_grid
...
def plot_clarke_error_grid(data_frame):
    total_gt_vals, total_pred_vals = [], []
    prediction = [x.strip('[ ]').split(',') for x in data_frame['prediction'].values]
    ground_truth = [x.strip('[ ]').split(',') for x in data_frame['ground_truth'].values]
    for pred, gt in zip(prediction, ground_truth):
        pred_values = [x.strip('[ ]').split(',') for x in pred]
        ...
        gt_values = [x.strip('[ ]').split(',') for x in gt]
        gt_values = data_to_float([z[0].split(" ") for z in gt_values])
        for g, p in zip(np.squeeze(gt_values), np.squeeze(pred_values)):
            total_gt_vals.append(g)
            total_pred_vals.append(p)
    plot, zone = clarke_error_grid(total_gt_vals, total_pred_vals, "Patient {}".format(patient_id))
    plt.savefig("{}/clarke_error_grid_{}.pdf".format(save_path, patient_id), dpi=600, bbox_inches='tight')
    ceg_zones = ["A", "B", "C", "D", "E"]
    ceg_results = {}
    for c, z in zip(ceg_zones, zone):
        ceg_results[c] = z / sum(zone)

    with open("ceg_results_{}.csv".format(patient_id), 'w') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=ceg_zones)
        writer.writeheader()
        writer.writerow(ceg_results)
